...
namespace {
    ...
    const char configFor[] = "VirtualFolders";  // identifying string for this plugin
...
void loadConfiguration() {
    ...
    filePath += L"\\VirtualFolders.json";
    ...
    if (saved.is_discarded()) {
        MessageBox(plugin.nppData._nppHandle,
            ...
            L"VirtualFolders", MB_ICONWARNING);
        configIgnored = true;
        return;
    }
    ...
    if (configFound) {
        if (configIgnored) {
            if (MessageBox(plugin.nppData._nppHandle,
                ...
                L"VirtualFolders", MB_YESNO | MB_ICONWARNING) == IDNO) return;
        }
        else {
            std::error_code ec;
            if (lastWriteTime != std::filesystem::last_write_time(filePath, ec)) {
                if (MessageBox(plugin.nppData._nppHandle,
                    ...
                    L"VirtualFolders", MB_YESNO | MB_DEFBUTTON2 | MB_ICONWARNING) == IDNO) return;
            }
        }
    }
...
void loadLocalization() {
    ...
    std::string externalLangFilePath = fromWchar(pluginPath) + "\\VirtualFolders\\localization\\" + langFileName;
